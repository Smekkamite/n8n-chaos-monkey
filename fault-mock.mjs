...
const baselineFor = (service) => {
  if (service === 'triage') return { message: { content: { ticket_id: 'break-my-workflow-mock-ticket', category: 'GENERAL_QUESTION', priority: 'LOW', sentiment: 'NEUTRAL', suggested_team: 'GENERAL_SUPPORT', knowledge_key: 'PASSWORD_RESET', automatic_resolution_recommended: true, human_review_recommended: false, confidence: 95, summary: 'Mock triage result', review_reason: '', status: 'OK' } } };
  if (service === 'response') return { message: { content: { subject: 'Mock support response', email_body: 'This draft was generated through the local Break My Workflow mock.' } } };
...
const server = http.createServer(async (request, response) => {
  ...
  if (url.pathname === '/__break_my_workflow/logs') return send(response, 200, { 'content-type': 'application/json' }, log);
  if (!url.pathname.startsWith('/mock/')) return send(response, 404, { 'content-type': 'application/json' }, { error: 'Use /mock/<service>' });

  const scenarioId = request.headers['x-break-my-workflow-scenario'] ?? url.searchParams.get('scenario');
...
server.listen(port, '127.0.0.1', () => console.log(`Fault mock listening at http://127.0.0.1:${port} with ${faults.size} faults.`));
