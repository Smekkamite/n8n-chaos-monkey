...
const readWebhookHeader = async (file) => {
  ...
  if (['content-type', 'x-chaos-test', 'x-chaos-campaign', 'x-chaos-scenario'].includes(header.name.toLowerCase())) {
    throw new Error(`Webhook header name ${header.name} is reserved by Chaos Monkey.`);
  }
...
function scenarios(payload, fields, expectations = {}) {
  ...
  const extra = scenarioPayload('unexpected_extra_field'); extra.__chaos_unexpected = { source: 'n8n-chaos-tester' };
...
async function request(target, req, marker = {}, options = {}) {
  const headers = { 'content-type': 'application/json', 'x-chaos-test': 'true', 'x-chaos-campaign': marker.campaignId ?? '', 'x-chaos-scenario': marker.scenarioId ?? '' };
...
function summarizeExecution(execution) {
  ...
  return { executionId: execution.id, status: execution.status, finished: execution.finished, chaos: { campaignId: headers['x-chaos-campaign'], scenarioId: headers['x-chaos-scenario'] }, nodes, errors: nodes.filter((node) => node.status === 'ERROR') };
}
...
async function collectExecutions() {
  ...
  do {
    ...
    traces = campaignId ? allTraces.filter((trace) => trace.chaos.campaignId === campaignId) : allTraces;
...
async function reportCampaign() {
  ...
  const rows = campaign.results.map((scenario) => {
    const matched = traces.filter((trace) => trace.chaos?.scenarioId === scenario.id);
  ...
  const markdown = [
    `# Chaos Monkey report — ${campaign.analysis.workflow}`,
    '',
  ...
  const out = opt('--out', 'chaos-report.md');
  await writeText(out, markdown);
...
const usage = () => `n8n-chaos ${VERSION}

Usage:
  n8n-chaos test --workflow workflow.json --payload payload.json --webhook URL [options]
...
async function runOneCommandTest() {
  ...
  const outDir = path.resolve(opt('--out-dir', path.join('chaos-results', stamp)));
...
async function main() {
  ...
  const campaignId = `chaos-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
